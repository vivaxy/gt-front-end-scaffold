...
const entryFileNameList = glob.sync(
    path.join(SOURCE_PATH, ENTRY_FOLDER_NAME) + '/*.js'
);
...
const htmlFileNameList = glob.sync(
    path.join(SOURCE_PATH, TEMPLATE_FOLDER_NAME) + '/*.html'
);
...
entryNameList.forEach(function(entryName) {
    webpackConfig.entry[entryName] = [
        path.join(__dirname, SOURCE_PATH, ENTRY_FOLDER_NAME, entryName + '.js')
    ];
    ...
    case ENV_DEVELOPMENT:
        entryNameList.forEach(function(entryName) {
            webpackConfig.entry[entryName].unshift(
                `webpack-dev-server/client?http://${config.DEVELOPMENT_IP}:${
                    config.DEVELOPMENT_PORT
                }`
            );
        ...
        webpackConfig.plugins.push(
            new webpack.ProgressPlugin((percentage, msg) => {
                logUpdate(
                    '     progress:',
                    numeral(percentage).format('00.00%'),
                    msg
                );
            })
        );
